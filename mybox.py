...
CAMERA = False  # Camera USB connectée au Raspberry PI 4
...
if not usb_camera.isOpened():
    LogEvent("Camera USB indisponible")
    CAMERA = False
else:
    CAMERA = True
    LogEvent("Camera USB installée")
...
####  Generation du flux camera USB ####

# Logitech webcam C270 maximum resolution : 1280x720
...
def graph():
    ...
    return render_template(
        "graph.html",
        data1=data1,  # humidité extérieur
        data2=data2,  # température extérieur
        data3=data3,  # température intérieur
        data4=data4,  # chauffage
        data5=data5,  # prise 240V
        data6=data6,  # Lumière
        **DATA
    )
...
def set_camera_state():
    ...
    Requête API sous forme :
    http://box.ppdlab.ch/setcamera?state=true
    """
...
def fan():
    ...
    if request.method == "POST":
        ...
        if state == "true":
            DATA["FAN"] = True
            # allumer le ventilateur (à faire)
            LogEvent("Ventilation du Raspberry PI enclenchée.")

        elif state == "false":
            DATA["FAN"] = False
            # éteindre le ventilateur (à faire)
